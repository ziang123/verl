...
try:
    from vllm.entrypoints.cli.serve import run_headless as run_vllm_server
except ImportError:
    from vllm.entrypoints.cli.serve import run_server as run_vllm_server
...
class vLLMHttpServer:
    ...
    async def launch_server(self, master_address: str = None, master_port: int = None, dp_rpc_port: int = None):
        ...
        if _VLLM_VERSION < version.parse("0.9.0"):
            # vLLM 0.8.x does not expose this newer CLI option.
    ...
    async def run_server(self, args: argparse.Namespace):
        ...
        # Don't keep the dummy data in memory. This helper exists in newer
        # vLLM releases but not in vLLM 0.8.x.
    ...
    async def generate(
        ...
        if not (_VLLM_VERSION < version.parse("0.9.0") and os.getenv("VLLM_USE_V1", "0") == "1"):
